...
export const createVA = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    try {
        ...
        logger.error(`Error creating va: ${(error as Error).message}`);
        next(error);
    }
};

export const vaOrderStatus = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    const { id } = req.params;
    try {
        ...
        // Handle unexpected errors
        logger.error(`Error fetching va status: ${(error as Error).message}`);
        next(error);
    }
};

export const createStaticVa = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    const partnerId = req.partnerId!;
    try {
        // Validate request payload
        ...
        // Handle unexpected errors
        logger.error(`Error creating va static: ${(error as Error).message}`);
        next(error);
    }
};
