...
export const createCreditCard = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    try {
        ...
        logger.error(`Error creating cc: ${(error as Error).message}`);
        next(error);
    }
};

export const ccOrderStatus = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    const { id } = req.params;
    try {
        ...
        // Handle unexpected errors
        logger.error(`Error fetching cc status: ${(error as Error).message}`);
        next(error);
    }
};
