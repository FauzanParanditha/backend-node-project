...
export const createXenditPaymentLink = async (order: Record<string, any>, next: NextFunction): Promise<any> => {
    try {
        ...
        logger.error(`Error creating Xendit payment link: ${(error as Error).message}`);
        next(error);
    }
};

export const xenditCallback = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
        ...
        logger.error(`Error handling webhook xendit: ${(error as Error).message}`);
        next(error);
    }
};

export const expiredXendit = async (id: string, next: NextFunction): Promise<any> => {
    try {
        const response = await xenditInvoiceClient.expireInvoice({ invoiceId: id });
        return response;
    } catch (error: unknown) {
        logger.error(`Error expiring Xendit invoice with ID: ${id}`, (error as Error).message);
        next(error);
    }
};

export const balance = async (_req: Request, res: Response): Promise<any> => {
