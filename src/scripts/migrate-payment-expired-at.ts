...
async function main() {
    await mongoose.connect(MONGO_URI as string, { autoIndex: false }); // index biar tidak re-build
    ...
    while (true) {
        ...
        for (const doc of docs) {
            processed++;
            lastId = doc._id as unknown as mongoose.Types.ObjectId;

            const dt = convertToDate((doc as Record<string, unknown>).paymentExpired as string);
            if (!dt) {
                skipped++;
                continue;
            }
