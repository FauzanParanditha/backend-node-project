...
import { initDailyCronReport } from "./cronDailyReport.js";

const TIMEZONE = "Asia/Jakarta";
dotenv.config();

// Initialize the 07:00 AM Daily Revenue Reporter
initDailyCronReport();
...
// helper concurrency
async function runInBatches<T>(items: T[], limit: number, worker: (item: T) => Promise<void>): Promise<void[]> {
    let i = 0;
    ...
    async () => {
        ...
        try {
            ...
            const legacyUpcoming = legacy.filter((o) => {
                const exp = convertToDate(o.paymentExpired as string);
            ...
            await runInBatches(candidates, PRECHECK_CONCURRENCY, async (o: any) => {
                logger.info({
                    tag: "PRECHECK.call",
                    orderId: o.orderId,
                    _id: String(o._id),
                ...
                try {
                    ...
                    logger.error(
                        "[PRECHECK] error per order:",
                        o?.orderId,
                        (e as AxiosLikeError)?.response?.data || (e as Error).message,
                    );
                }
            });
        } catch (err) {
        ...
        try {
            ...
            if (dueIds.length) {
                ...
                for (const o of expiredDocs) {
                    const ts = (o.paymentExpiredAt ?? convertToDate(o.paymentExpired as string))?.toISOString?.();
