...
function isQris(order: OrderLike): boolean {
    const m = (order?.paymentMethod || "").toLowerCase();
    const t = (order?.paymentType || "").toLowerCase();
    return m === "qris" || t === "qris" || Boolean(order?.qris);
}

function normalizeStatus(payload: Record<string, any>): "PAID" | "PENDING" | "FAILED" | "EXPIRED" {
    const raw =
        payload?.status ??
...
function normalizePaidAt(payload: Record<string, any>): Date | null {
    const cand =
        payload?.paidAt ??
        payload?.paid_at ??
 ...
 */
export async function getProviderStatus(order: OrderLike): Promise<ProviderStatusResult> {
