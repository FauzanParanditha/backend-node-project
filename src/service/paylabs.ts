...
export const minifyJson = (body: Record<string, unknown>): string => {
    ...
    if (parsed.payer) {
        parsed.payer = (body as Record<string, unknown>).payer; // Retain original spacing in `payer`
    }
...
export const createSignature = (
    ...
    try {
        ...
        logger.error(`Failed to read private key: ${message}`);
        return null;
    }
...
export const verifySignature = (
    ...
    try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        logger.error(`Failed to read public key: ${message}`);
        return false;
    }
...
export const verifySignatureForward = (
    httpMethod: string,
    endpointUrl: string,
    body: Record<string, unknown>,
    timestamp: string,
...
export const verifySignatureMiddleware = async (
    ...
    try {
        ...
        const ageInMinutes = Math.abs((currentTime.getTime() - requestTime.getTime()) / (1000 * 60));
        ...
        logger.error(`Error during signature verification: ${err.message}`);
        return false;
    }
};
...
export const generateHeaders = (
    ...
    let signature: string | false | null;
    try {
        signature = createSignature(method, endpoint, requestBody, timestamp);
    } catch (error: unknown) {
        const err = error instanceof Error ? error : new Error(String(error));
        logger.error(`❌ Failed to generate signature: ${err.message}`);
...
export const generateHeadersForward = (
    method: string,
    endpoint: string,
    requestBody: Record<string, unknown>,
    requestId: string,
...
export const handlePaymentLink = async (orderData: OrderData): Promise<Record<string, unknown>> => {
    let paymentLink: PaymentLinkResponse | undefined;
    switch (orderData.paymentMethod) {
        case "xendit": {
            const noopNext = () => {};
            paymentLink = (await createXenditPaymentLink(orderData, noopNext)) as PaymentLinkResponse | undefined;
            break;
        }
        case "paylabs":
            paymentLink = (await createPaymentLink(orderData)) as PaymentLinkResponse | undefined;
            break;
        default:
...
export const convertToDate = (paymentExpired: string | number): Date | null => {
