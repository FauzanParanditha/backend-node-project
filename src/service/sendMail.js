...
export const SendEmail = async (mailData) => {
    if (!process.env.MAIL_TOKEN || !process.env.MAIL_FORM || !process.env.MAIL_ADDRESS || !process.env.MAIL_URL) {
    ...
    try {
        ...
        logger.error("Error sending email:", error?.response);
        ...
        try {
            await email.save();
            logger.log("Email log created successfully.");
        } catch (logError) {
